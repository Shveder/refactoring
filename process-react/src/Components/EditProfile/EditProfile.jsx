...
function EditProfile() {
    ...
    function handleEditLogin(newLogin) {
        ...
        axios.put("https://localhost:44367/User/ChangeLogin", requestData)
            .then(response => {
                if (response.data === "Login changed") {
                    setLoginText("Логин успешно изменен на " + newLogin);
                }
                if (response.data.message === "There is already a user with this login in the system") {
                    setLoginText("Пользователь с таким логином уже есть в системе");
                }
            })
            .catch(error => {
                console.log(error);
                setLoginText(error.response.data.message);
            });
    }
    function handleEditPassword(password, newPassword) {
        ...
        axios.put("https://localhost:44367/User/ChangePassword", requestData)
            .then(response => {
            ...
            .catch(error => {
                console.log(error);
                setPasswordText(error.response.data.message);
            });
    }
